...
| Service disclosures | relevant business identity, terms, privacy, subscription, cancellation, or refund information removed, hidden, misleading, or absent from a service draft | accessible disclosures and flows that match the service's actual market, data handling, and sales model |
...
- Preserve business identity, terms, privacy, subscription, cancellation, refund, and other applicable service disclosures. Do not remove, hide, or treat them as visual clutter without confirming that they are out of scope.
- For a Korean service draft, identify whether the service handles personal data, uses account creation, sells directly to consumers, or includes recurring billing. Include the applicable disclosure and navigation structure early, rather than adding it as a final visual patch.
- When a draft is explicitly a concept and the required business or policy details are unavailable, use clearly labeled placeholders or a handoff checklist for the needed information. Do not invent company details, registration numbers, policy terms, prices, refund rules, or legal compliance claims; do not ship empty or deceptive disclosure links.
- For consumer-facing paid subscriptions, make the service provider identity, price and billing cycle, automatic-renewal terms, cancellation and refund path, customer support, and terms easy to find before purchase and from the footer. Do not apply consumer-sales disclosures automatically to a documented B2B-only or free service.
